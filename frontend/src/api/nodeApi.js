...
export async function fetchNodesByNameFragmentWithoutLabel(
  query,
  skip = 0,
  limit = 20,
) {
  const url = `${BASE_URL}/nodes/search/by-name?query=${
  ...
  try {
    ...
    console.error(
      `Erreur lors de la récupération des noeuds : ${error.message}`,
    );
    return [];
  }
}
...
export async function fetchMetrics({ startDate, endDate }) {
  const response = await fetch(
    `${BASE_URL}/metrics?startDate=${encodeURIComponent(startDate)}&endDate=${
